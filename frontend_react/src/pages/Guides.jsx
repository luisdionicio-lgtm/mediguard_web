...
/* Mapea una guía real del backend a su detalle interactivo estático
   (frontend_react/src/data/learnGuides.js) cuando el título coincide con
   uno de los temas que ya tienen contenido completo. Si no hay match,
   el botón queda deshabilitado en vez de simular una acción que no pasa nada. */
const INTERACTIVE_GUIDE_MATCHERS = [
  { slug: 'quemaduras',           test: (title) => /quemadur/i.test(title) },
  { slug: 'control-hemorragias',  test: (title) => /hemorrag|sangr/i.test(title) },
  { slug: 'rcp',                  test: (title) => /\brcp\b|reanimaci[oó]n cardiopulmonar|paro card[ií]aco/i.test(title) },
];

function matchInteractiveSlug(title) {
  const match = INTERACTIVE_GUIDE_MATCHERS.find((m) => m.test(title || ''));
    ...
    <div className="page-container">
      ...
      <div className="resource-grid stagger">
        {guides.map((guide, i) => {
          ...
          const interactiveSlug = matchInteractiveSlug(guide.title);
          return (
              ...
              <div className="dash2-guide-body">
                ...
                {interactiveSlug ? (
                  ...
                  <button
                    ...
                    style={{ marginTop: '1.25rem' }}
                    disabled
                    title="Guía interactiva próximamente disponible"
                  >
                    Iniciar Guía Interactiva
                  </button>
                )}
